...
export class ProductWidgetStore {
  ...
  private reportBIOnAppLoaded() {
    ...
    this.siteStore.biLogger.productWidgetLoadedSf(
      this.product
        ? {
  ...
  public async setInitialState(): Promise<void> {
    const externalResponses = await Promise.all([
      ...
      this.productApi.getData(this.externalId || '', this.compId, this.shouldRenderPriceRange),
  ...
  private sendClickShippingInfoLinkSf() {
    // eslint-disable-next-line @typescript-eslint/no-floating-promises
    this.siteStore.biLogger.clickShippingInfoLinkSf({productId: this.product.id});
  }
