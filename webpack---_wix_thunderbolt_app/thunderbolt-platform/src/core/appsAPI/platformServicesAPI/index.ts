...
export const createPlatformAppServicesApi = ({
	...
	const biLoggerFactory = () =>
		biLoggerFactoriesCreator.createBaseBiLoggerFactory().updateDefaults({
	...
	const fedOpsLoggerFactory = createFedopsLogger({
		biLoggerFactory: biLoggerFactoriesCreator.createBiLoggerFactoryForFedops().updateDefaults({
			_appId: appDefinitionId,
			_instanceId: instanceId,
