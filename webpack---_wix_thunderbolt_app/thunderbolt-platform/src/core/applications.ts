...
import type { BlocksAppsUtils } from './blocksAppsUtils'
import type { DataBindingViewerAppUtils } from './dataBindingViewerAppUtils'
...
import { PlatformDebugApi } from './debug'
...
export function Applications({
	...
	const runApplication = async (appDefinitionId: string) => {
		...
		const wixCodeApi = await logger.runAsyncAndReport('init_wix_code_apis', () => wixCodeApiFactory.initWixCodeApiForApplication(appDefinitionId))
		...
		await Promise.all(
			controllerPromises.map(async (controllerPromise, index) => {
				...
				if (controllerParams.appParams.appDefinitionId === BlocksPreviewAppDefId && controller.exports) {
					handlers.controllers.registerExportsSetter(controllerCompId, (props: Record<string, any>) => {
						Object.assign(controller.exports!() as Record<string, any>, props)
					})
				}
				if (controller.updateConfig && isEditorMode) {
					handlers.controllers.registerToConfigUpdate(controllerCompId, (updatedConfig: unknown) => controller.updateConfig!(controllerParams.$w, updatedConfig))
				}
			})
		)
	}
