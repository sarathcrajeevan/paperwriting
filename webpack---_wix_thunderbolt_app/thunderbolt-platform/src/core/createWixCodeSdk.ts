...
import type { PlatformEnvData, PlatformUtils, WixCodeApi, FeatureName, PlatformLogger, ClientSpecMapAPI, ModelsAPI, WixCodeNamespacesRegistry, ModuleLoader } from '@wix/thunderbolt-symbols'
...
export function createWixCodeApiFactory({
	...
	const registerDefaultWixCodeApi = _.once(async (factories: Array<SdkFactory>) => {
		const defaultWixCodeApi = Object.assign({}, internalNamespaces, ...(await Promise.all(_.map(factories, (factory) => factory()))))
		wixCodeNamespacesRegistry.registerWixCodeNamespaces(defaultWixCodeApi)
	})
	...
	return {
		initWixCodeApiForApplication: async (appDefinitionId: string) => {
			...
			await registerDefaultWixCodeApi(factories)
