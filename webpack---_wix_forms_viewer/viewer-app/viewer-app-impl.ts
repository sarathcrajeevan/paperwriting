...
import { AddressController } from './controllers/complex-fields/address-controller'
import { PhoneController } from './controllers/complex-fields/phone-controller'
import { getController, IController } from './controllers/controllers'
import { CorvidAPI } from './corvid-api'
import { ErrorName, FieldValidity, getRelevantError } from './errors'
import { getPhoneStringValueByType } from './field-dto/field-dto'
import { LIMIT_VIOLATION_ERROR } from './services/constants'
import { isLimitReached } from './services/form-limit-services'
import { isPaymentAllowed } from './services/payment-services'
import translations from './services/translations'
import { siteStore } from './stores/site-store'
import { submitUtils } from './submit-utils'
import { getAppVersion, getViewerSentryDSN } from './utils/utils'
import {
  componentStringify,
  escapeRegExp,
  findPlugin,
...
const onSubmit = async ({ $w, controller, corvidAPI, isPaymentForm }: SubmitArgs) => {
  ...
  const postSubmitActions = (shouldShowSubmissionSuccess = true, attachments = []) => {
    ...
    if (!isTemplate(wixLocation)) {
      submitUtils.sendWixAnalytics({ wixSeo, wixWindow })
    }
  }
...
export const createControllers = (controllerConfigs) =>
  controllerConfigs.map((controllerConfig) => {
    const { $w, type, wixCodeApi }: { $w; type: ControllerType; wixCodeApi } = controllerConfig
    switch (type) {
      case 'complexPhoneController': {
        return Promise.resolve(new PhoneController($w).controller)
      }
      case 'complexAddressController': {
        return Promise.resolve(new AddressController($w).controller)
      }
      default: {
