...
class SubmitUtils {
  ...
  public async getAttachments(fields): Promise<Attachment[][]> {
    ...
    try {
      const attachments: Attachment[][] = await Promise.all(
        uploadButtons.map(async (uploadButtonField): Promise<Attachment[]> => {
          const useMultipleFieldsAPI = siteStore.isEnabled(
            'specs.crm.FormsViewerMultipleFileUploadSupport',
          )
          if (useMultipleFieldsAPI) {
            const files = await uploadButtonField.uploadFiles()
            return files.map(
              (
                { fileUrl: url, width, height, fileName: mediaId, originalFileName: title },
                idx,
              ) => ({
                url,
                name: uploadButtonField.value[idx].name,
                value: '',
  ...
  public sendWixAnalytics({ wixWindow, wixSeo }) {
