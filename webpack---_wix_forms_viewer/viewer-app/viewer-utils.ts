...
export const getFieldValue = ({
  ...
  switch (fieldType) {
    ...
    case FIELD_TYPE.FILE_UPLOAD: // we need this for multi-step form
      const fieldAttachments = getAttachment(field, attachments)
      return fieldAttachments?.length
        ? fieldAttachments.map(({ url, mediaId, title, width, height }) => ({
            url,
            mediaId,
            title,
            width,
    ...
    case FIELD_TYPE.SIGNATURE: // we need this for multi-step form
      const signatureAttachments = getAttachment(field, attachments)
      return signatureAttachments?.length ? signatureAttachments[0].value : ''
    default:
      return field.value
  }
}
