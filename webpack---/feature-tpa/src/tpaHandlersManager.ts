...
export const TpaHandlersManager = withDependencies(
	...
	(
		...
		const isFaqTemp = (origin: string) => origin === 'https://v3-170-0-dot-wixlabs-wix-faq-11.uc.r.appspot.com'
		...
		return {
			async handleMessage({ source: tpa, origin, message }) {
				...
				if (
					...
					!isTestOrigin(origin) &&
					!isFaqTemp(origin) // TODO temp hack for https://wix.slack.com/archives/CAKBA7TDH/p1635170161006400
				) {
