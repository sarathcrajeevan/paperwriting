...
var _require = require('./imageServiceUtils'),
    ...
    getScaleFactor = _require.getScaleFactor,
    getOverlappingRect = _require.getOverlappingRect,
    getAlignedRect = _require.getAlignedRect,
    getTransformData = _require.getTransformData,
    getFocalPoint = _require.getFocalPoint;
/**
...
function setTransformParts(transformsObj, src, target) {
    var rect; // crop source image if needed
    ...
    switch (transformsObj.fittingType) {
        ...
        case constants.fittingTypes.ORIGINAL_SIZE:
            ...
            rect = getAlignedRect(transformsObj.src, transformsObj.focalPoint, target, target.alignment);
...
function getFitPart(transformsObj, target) {
    ...
    var transformedData = getTransformData(transformsObj.src.width, transformsObj.src.height, target, transformsObj.devicePixelRatio, constants.transformTypes.FIT, transformsObj.upscaleMethod);
    var transformType = constants.transformTypes.FILL; // return fit transform data
...
function getFillPart(transformsObj, target) {
    // calculate the transformed image size needed
    var transformedData = getTransformData(transformsObj.src.width, transformsObj.src.height, target, transformsObj.devicePixelRatio, constants.transformTypes.FILL, transformsObj.upscaleMethod);
    var focalPoint = getFocalPoint(transformsObj.focalPoint);
    var transformType = focalPoint ? constants.transformTypes.FILL_FOCAL : constants.transformTypes.FILL; // return fill transform data
...
function getStretchPart(transformsObj, target) {
    // stretch data
    var scaleFactor = getScaleFactor(transformsObj.src.width, transformsObj.src.height, target.width, target.height, constants.transformTypes.FILL);
    var clonedTarget = Object.assign({}, target);
...
module.exports.setTransformParts = setTransformParts;
