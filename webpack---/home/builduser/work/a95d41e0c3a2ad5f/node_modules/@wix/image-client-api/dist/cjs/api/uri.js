...
var transforms = require('../engines/transforms');
...
function getURI(fittingType, src, target, options, transformObj) {
    var uri = constants.emptyData.uri; // check if image transformation is applicable (e.g. .gif, .wix_mp)

    if (isImageTransformApplicable(src.id)) {
        transformObj = transformObj || transform.getTransform(fittingType, src, target, options, transformObj); // set the uri property
