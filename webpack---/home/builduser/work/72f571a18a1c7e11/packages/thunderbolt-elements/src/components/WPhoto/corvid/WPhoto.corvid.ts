...
const _wPhotoSDKFactory: IWPhotoOwnSDKFactory = ({
  ...
  return {
    ...
    set fitMode(value) {
      const { maskDataElementStringNotStretch } = sdkData;
      const maskDataElementProps = maskDataElementStringNotStretch
        ? { maskDataElementString: maskDataElementStringNotStretch }
        : {};
      setProps({
        displayMode: IMAGE_FIT_MODE_MAP[value],
        ...maskDataElementProps,
      });
